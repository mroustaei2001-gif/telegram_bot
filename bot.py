...
# State ها برای مدیریت وضعیت کاربر
(
...
async def check_membership(user_id, context):
    """بررسی عضویت در کانال اصلی و کانال‌های تبلیغاتی"""
    ...
    for channel in ads_channels:
        if channel[3]:  # is_active
            ...
            try:
                ...
                logger.error(f"Ads channel check error: {e}")
                continue
    
    return True

async def get_join_message(context):
    """ساخت پیام عضویت اجباری"""
    messages = []
    
    main_channel = get_setting('main_channel_username')
    if main_channel:
        messages.append(f"📢 کانال اصلی: @{main_channel}")
    
    ads_channels = get_all_ads_channels()
    for channel in ads_channels:
        if channel[3]:  # is_active
            messages.append(f"📢 کانال تبلیغاتی: @{channel[2]}")
    
    if not messages:
        return "⚠️ هیچ کانالی برای عضویت اجباری تنظیم نشده است."
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بررسی عضویت برای همه عملیات‌ها (به جز انصراف و بررسی عضویت)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == ADMIN_WAITING_NEW_AD_CHANNEL and is_admin(user.id):
        ...
        try:
            ...
            add_ads_channel(str(chat.id), channel_username)
            await update.message.reply_text(
            ...
            await update.message.reply_text(
                ...
                f"لطفاً مطمئن شوید کانال معتبر است و ربات ادمین آن است.",
                reply_markup=admin_panel_keyboard()
            )
        return

    # ─── ادمین: حذف کانال تبلیغاتی ───
...
async def admin_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_users":
        ...
        for user in users[:20]:  # محدود به 20 کاربر
    ...
    if data == "admin_ideas":
        ...
        for idea in ideas[:15]:  # محدود به 15 ایده
    ...
    if data == "admin_toggle_mandatory":
        ...
        await admin_handler(update, context)  # بازگشت به منوی عضویت اجباری
        return
    ...
    if data == "admin_add_config":
        context.user_data['state'] = ADMIN_WAITING_NEW_PROXY  # همان state
    ...
    if data == "admin_channel":
        ...
        text += "⚠️ توجه: ربات باید ادمین کانال باشد."
